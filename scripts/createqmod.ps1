...
foreach ($mod in $modJson.modFiles) {
    ...
    if (-not (Test-Path $path)) {
        Write-Output "Error: could not find dependency: $mod"
        exit 1
    }
    $filelist += $path
}
...
foreach ($lib in $modJson.libraryFiles) {
    ...
    if (-not (Test-Path $path)) {
        Write-Output "Error: could not find dependency: $lib"
        exit 1
    }
    $filelist += $path
}
...
foreach ($copy in $modJson.fileCopies) {
    ...
    if (-not (Test-Path $path)) {
        $path = "./extern/libs/" + $copy
    }
    if (-not (Test-Path $path)) {
        Write-Output "Error: could not find file copy: $copy"
        exit 1
    }
    $filelist += $path
}
